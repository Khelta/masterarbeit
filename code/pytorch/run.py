...
def run(algorithm, dataset, ap, epochs, cop=None, cycles=5, historun=False):
    ...
    if cop is None: 
        if historun:
            raise ValueError("Insert cop for --historun")
        ...
        labels = globals()["LABELS_" + dataset.upper()]
        if historun: cycles = 1
        for cycle in range(cycles):
            for label in labels:
                if historun:
                    dir_path = "results/historun/{}/{}/{}/".format(algorithm, dataset, cycle)
                else:
                    dir_path = "results/{}/{}/{}/".format(algorithm, dataset, cycle)
                file_name = "{}-{}-{}-{}-{}-{}".format(dataset, label, ap, cop, epochs, algorithm)
                file_prefix = dir_path + file_name
                ...
                print("Cycle:", cycle)
                if not os.path.exists(file_prefix+"-train-loss.csv"):
                    complete_run_cae(dataset, algorithm, file_prefix, label, cop, ap, epochs, historun)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-cop", help="the percentage of cut-off", type=restricted_float, default=None)
